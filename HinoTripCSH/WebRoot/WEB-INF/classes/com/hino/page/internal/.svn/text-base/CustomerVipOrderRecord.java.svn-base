...
public class CustomerVipOrderRecord extends BasePage{
	@Bindable
    ...
    public ActionLink confirmLink = new ActionLink("confirm", "发卡完毕");
    public ActionLink cardletterLink = new ActionLink("cardletter", "生成发卡信");
    @Override
    public void onInit() {
    	confirmLink.setActionListener(new ActionListener() {
            private static final long serialVersionUID = 1L;
            public boolean onAction(Control source) {
            	ServiceResponse sr = getCustomerService().vip_card_completed(confirmLink.getValueInteger());
            	if(sr.isSucc())
            	{
    	...
    	cardletterLink.setActionListener(new ActionListener() {
            private static final long serialVersionUID = 1L;
            public boolean onAction(Control source) {
            	setRedirect("./card_letter.htm?vid="+cardletterLink.getValueInteger());
                return true;
            }
        });
		...
		column = new Column("delivery_method", "发卡方式");
		column.setDecorator(new Decorator(){
			@Override
			public String render(Object object, Context context) {
				
				return Info.VCM_STR_CN[((VipOrder) object).getDelivery()];
			}
		});
		vip_table.addColumn(column);
		...
		ActionLink[] links = {confirmLink, cardletterLink,deleteLink}; 
